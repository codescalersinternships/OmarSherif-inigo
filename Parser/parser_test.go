...
func TestParser(t *testing.T) {
	...
	t.Run("returns the section dictionary", func(t *testing.T) {
		...
		want := sectionDictionary{map[string]Section{"owner": Section{"owner", []string{}, []string{"name", "organization"},
			map[string]string{"name": "John Doe", "organization": "Acme Widgets Inc."}},
			"database": Section{"database", []string{"	; use IP address in case network name resolution is not working"},
